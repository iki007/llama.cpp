...
// the end of the sorted buffer read as zero; tokens past the tile's count are computed, not stored).
...
ESIMD_INLINE void mul_mat_id_dpas(const void * weights, const size_t expert_bytes, const int ncols, const int nrows,
                                  const sycl::half * y, const int y_rows, float * dst,
...
#pragma unroll
    for (int i = 0; i < NG * 8; ++i) {
        if (i < t.row_count) {
            float * d = dst + (size_t) (t.row_begin + i) * nrows + row0;
            if (row0 + ROWS <= nrows) {
...
static void mul_mat_id_dpas_sycl(const void * weights, size_t expert_bytes, int ncols, int nrows, const sycl::half * y,
                                 int y_rows, float * dst, const ggml_sycl_mmid_tile * tiles, int n_tiles,
    ...
    stream->parallel_for(sycl::nd_range<1>(sycl::range<1>(wgs * TPW), sycl::range<1>(TPW)),
                         [=](sycl::nd_item<1> it) [[intel::sycl_explicit_simd]] {
                             mul_mat_id_dpas<T, NG>(weights, expert_bytes, ncols, nrows, y, y_rows, dst, tiles,
                                                    n_row_wgs, it);
                         });
}
...
void ggml_sycl_mul_mat_id_dpas(ggml_type type, const void * weights, size_t expert_bytes, int ncols, int nrows,
    ...
    switch (type) {
        case GGML_TYPE_Q4_K:
            mul_mat_id_dpas_sycl<GGML_TYPE_Q4_K, 4>(weights, expert_bytes, ncols, nrows, y, y_rows, dst, tiles, n_tiles, stream);
            break;
        case GGML_TYPE_Q6_K:
            mul_mat_id_dpas_sycl<GGML_TYPE_Q6_K, 4>(weights, expert_bytes, ncols, nrows, y, y_rows, dst, tiles, n_tiles, stream);
            break;
        default:
    ...
    GGML_UNUSED(y); GGML_UNUSED(y_rows); GGML_UNUSED(dst); GGML_UNUSED(tiles); GGML_UNUSED(n_tiles); GGML_UNUSED(stream);
